  ...
  return (
    <>
      <div className='blackBox'>
        <Suspense fallback={<div className={classNames('loadingAnim position-relative', albumClassnames)} />}>
          {comments.length > 0
            ? (
              <div className={albumClassnames} >
                {isMultiple ? <SideButton side='left' onClick={() => setCurrentIndex(pastIndex)} /> : null}
                ...
                {isMultiple ? <SideButton side='right' onClick={() => setCurrentIndex(nextIndex)} /> : null}
              </div>
            )
            : null}
        </Suspense>
          ...
          {isFAU
              ...
              <>
                <ModalTemplate id={modalId}>
                  <form onSubmit={submit}>
                    <div className='row'>
                      <div className='col'>
                        <textarea className='form-control' required name='text' maxLength={300} onChange={ev => setDefaultValue(ev.target.value)} defaultValue={defaultValue} />
                      </div>
                    </div>
                    <div className='row mt-2'>
                      <div className='col'>
                        <div className="form-check">
                          <input className="form-check-input" type="checkbox" defaultChecked={selfComment ? selfComment.anon : false} name='anon' />
                          <label className="form-check-label" htmlFor="flexCheckDefault">{t('Comment_Anon')}</label>
                        </div>
                      </div>
                    </div>
                    <div className='row mt-2'>
                      <div className='col mx-auto'>
                        <SubmitButton loading={loading}>{t('Save comment')}</SubmitButton>
                      </div>
                    </div>
                  </form>
                </ModalTemplate>
                <div className='col col-3'>
                  <button onClick={() => showModal(modalSelector)} className='btn btn-outline-light w-100 rounded-3' style={{ fontSize: '18px' }}>
            ...
            : (
              <div className='col col-4'>
                <button onClick={() => showModal('#loginModal')} className='btn btn-outline-light w-100 rounded-3' style={{ fontSize: '18px' }}>
                  {t('Comment_Login')}
                </button>
              </div>
            )}
